...
`r q <- q+1; letters[q]`. Read in the data and store it as an object called `school`. (**1 Points**)

```{r}
# Your code here...
```

`r q <- q+1; letters[q]`. Change the column names to the following: (**1 Points**)
...
`r q <- q+1; letters[q]`. Delete the `income` variable and calculate the percent of students who attended catholic school. (**1 Points**)

```{r}
# Your code here...
```

`r q <- q+1; letters[q]`. Use `modelsummary` to create a summary statistics table for students who attended catholic school. (**1 Points**)

```{r}
# Your code here...
```

`r q <- q+1; letters[q]`. Use `modelsummary` to create a summary statistics table for students who did not attend catholic school. (**1 Points**)
...
`r q <- q+1; letters[q]`. Do you think income in an important variable to control for? Why, or why not? Use "DAG" language in your answer. (**1 Points**)

`r q <- q+1; letters[q]`. Regardless of your thoughts about whether income is important, what are some other factors (that may or may not exist in the dataset) that might be important to control for?  Again, use "DAG" language in your answer. (**1 Points**)
...
`r q <- q+1; letters[q]`. Using this model, generate propensity scores as a new variable called `ps` in `school`. (**1 Points**)

```{r}
# Your code here...
```

`r q <- q+1; letters[q]`. Using these propensity scores, create inverse probability weights as a new variable called `w` in `school`. (**1 Points**)
...
`r q <- q+1; letters[q]`. Estimate the model parameters with OLS. (**2 Points**)

```{r}
# Your code here...
```

`r q <- q+1; letters[q]`. Use the `MatchIt` package in R to create a matched sample (use `method = "nearest"`).  Then, estimate the parameters with OLS using the matched sample. (**2 Points**)

```{r}
# Your code here...
```

`r q <- q+1; letters[q]`. Estimate the parameters OLS and inverse probability weights. (**2 Points**)

```{r}
# Your code here...
```

`r q <- q+1; letters[q]`. Display the coefficients of these models in a table generated by `modelsummary`. (**1 Points**)
