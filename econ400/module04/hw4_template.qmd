...
`r q <- q+1; letters[q]`. Read in the data and store it as an object called `school`. (**2 Points**)
...
`r q <- q+1; letters[q]`. Calculate the percent of students who attended catholic school. (**2 Points**)

```{r}
# Your code here...
```

`r q <- q+1; letters[q]`. Use `modelsummary` to create a summary statistics table for students who did not attend catholic school. (**2 Points**)

```{r}
# Your code here...
```

`r q <- q+1; letters[q]`. Use `modelsummary` to create a summary statistics table for students who attended catholic school. (**2 Points**)
...
`r q <- q+1; letters[q]`. Using this model, generate propensity scores as a new variable called `ps` in `school`. (**2 Points**)

```{r}
# Your code here...
```

`r q <- q+1; letters[q]`. Using these propensity scores, create inverse probability weights as a new variable called `w` in `school`. (**2 Points**)

```{r}
# Your code here...
```

`r q <- q+1; letters[q]`. Using these weights, re-create the income distributions from before.  Comment on any similarities/differences.^[Hint: `density()` contains an option for weights.] (**4 Points**)
...
`r q <- q+1; letters[q]`. Use the `MatchIt` package in R to create a matched sample (match based on propensity scores).  Then, estimate the parameters with OLS using the matched sample. Store these as `r_match1` and `r_match2`. Again, hold off on displaying these coefficients. (**3 Points**)
