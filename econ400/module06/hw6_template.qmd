...
`r q <- q+1; letters[q]`. Explain the identification strategy used in this paper. (**3 Points**)

*Write your response here.*

`r q <- q+1; letters[q]`. Draw a (simplified) DAG for this setting, or write down the first and second stage regressions used in the IV estimation in the notation used in the notes. (**3 Points**)
...
`r q <- q+1; letters[q]`. Plot the average per-capita funding by years of potential participation. Does this figure support the authors candidate for an instrument? (**3 Points**)
...
`r q <- q+1; letters[q]`. Interpret the primary coefficients of interest as if they were causal. (**3 Points**)

*Write your response here.*

`r q <- q+1; letters[q]`. What argument(s) do the authors make for why we cannot interpret these estimates as causal? (**3 Points**)

*Write your response here.*

`r q <- q+1; letters[q]`. Could the author(s) have estimated a difference-in-differences? Why or why not (aside from the data not being in a panel format)? (**3 Points**)
...
`r q <- q+1; letters[q]`. Estimate the reduced form relationship between the instrument and the previous outcomes (conditional on the control variables from the previous question). These estimates should match the estimates on the first row of Table 2. Present the estimates in a table. (**3 Points**)
...
`r q <- q+1; letters[q]`. Estimate the effect of per-capita funds on the different outcomes in Table 3 using IV. Display the coefficients in a table using `modelsummary`. (**3 Points**)
