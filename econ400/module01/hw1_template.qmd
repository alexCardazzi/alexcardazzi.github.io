...
`r q <- q+1; letters[q]`. Use the `density()` function to plot the wage distribution for husbands of employed and unemployed women in 1975. What do you learn from this figure? (**3 Points**)
...
`r q <- q+1; letters[q]`. Tabulate and then visualize the *total* number of children for the women sampled. The x-axis should contain # of children and the y-axis should measure the number of women with that number of children. What do you learn from this figure? (**3 Points**)
...
`r q <- q+1; letters[q]`. Calculate the average number of young and old children (separately) by age.  Plot the relationship between children and age. What do you learn from this figure? (**3 Points**)
...
`r q <- q+1; letters[q]`. Generate a scatterplot between a woman's wages and her husband's wages. Be sure to include axis labels, etc. Add a diagonal line that denotes y = x. What are some things you can learn from this plot? (**3 Points**)
...
`r q <- q+1; letters[q]`. Re-estimate the model using only the subset of employed women. (**2 Points**)

```{r}
# Your code here...
```

`r q <- q+1; letters[q]`. Display the coefficients from both estimations in a single table. (**3 Points**)

```{r}
# Your code here...
```

`r q <- q+1; letters[q]`. Interpret the coefficients in the regressions. (**4 Points**)

*Your answer here...*


`r q <- q+1; letters[q]`. Do any of the coefficients change between regressions? Why or why not? Which regression do you prefer, and why? (**3 Points**)
...
`r q <- q+1; letters[q]`. Interpret each coefficient in the regression. (**4 Points**)
