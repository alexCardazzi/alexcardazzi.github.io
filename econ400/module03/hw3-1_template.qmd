...
`r q <- q+1; letters[q]`. Using `table()`, find the number of observations in each of the following groups: treatment group pre-treatment, treatment group post-treatment, control group pre-treatment, control group post-treatment. (**2 Points**)
...
`r q <- q+1; letters[q]`. Using `aggregate()`, find the average number of new operas for each of the states. (**2 Points**)
...
`r q <- q+1; letters[q]`. How do the authors address these assumptions in the paper (if at all)? Discuss the potential threats to a causal interpretation of the results in the paper. (**6 Points**)
...
Do your best to replicate the content in Figure 2 using `italy`. *Note*: the lines in this graph might not replicate perfectly, but do what you can. (**6 Points**)
...
`r q <- q+1; letters[q]`. Estimate a 2x2 DiD model of the form (**2 Points**):
...
`r q <- q+1; letters[q]`. Replicate the results in Columns (1) and (2) on Table 3 in the paper.  Do not worry about replicating the standard errors.^[You only need to replicate the first two columns.] (**3 Points**)
...
`r q <- q+1; letters[q]`. Create a table using `modelsummary` containing coefficients from the three models. (**3 Points**)
...
Replicate the Event Study in Figure 3 of the paper. (**5 points**)
...
Do you believe the result to be credible in terms of causality and magnitude? Why or why not? (**5 Points**)
