...
`r q <- q+1; letters[q]`. Regardless of your answer to the previous question, what are some other factors (that may or may not exist in the dataset) that might be important to control for?  Again, use "DAG" language in your answer. (**1 Point**)
...
`r q <- q+1; letters[q]`. Use the `MatchIt` package in R to create a matched sample (match based on propensity scores).  Then, estimate the parameters with OLS using the matched sample. Store these as `r_match1` and `r_match2`. Again, hold off on displaying these coefficients. (**1 Point**)
