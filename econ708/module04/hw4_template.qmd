...
Explore whether catholic school education improves outcomes.  The data for this homework can be found [here](https://alexcardazzi.github.io/econ708/data/catholic2.csv) ([codebook](https://alexcardazzi.github.io/econ708/data/ecls-codebook.txt)).

`r q <- q+1; letters[q]`. Read in the data and store it as an object called `school`. (**0 Points**)

```{r}
# Your code here...
```

`r q <- q+1; letters[q]`. Change the column names to the following: (**0 Points**)
...
`r q <- q+1; letters[q]`. Delete the `income` variable and calculate the percent of students who attended catholic school. (**1 Point**)
...
`r q <- q+1; letters[q]`. Use `density()` to estimate the distributions of the log of `income2` for students who attend catholic school and students who do not.  Then, plot these two densities on the same set of labeled axes.  Remember to ensure both distributions are fully visible (not cutoff), different colors, and identified in a legend. (**2 Points**)

```{r}
# Your code here...
```

`r q <- q+1; letters[q]`. Do you think income in an important variable to control for? Why, or why not? Use "DAG" language in your answer. (**1 Point**)

`r q <- q+1; letters[q]`. Regardless of your thoughts about whether income is important, what are some other factors (that may or may not exist in the dataset) that might be important to control for?  Again, use "DAG" language in your answer. (**1 Point**)
...
`r q <- q+1; letters[q]`. Estimate a propensity score model using mother's age, mother's education, and family income as predictors.^[Remember: propensity score models are often estimated using logistic regression.] (**1 Point**)
...
`r q <- q+1; letters[q]`. Interpret the signs of the model's estimated coefficients. (**0 Points**)

*Your answer here...*

`r q <- q+1; letters[q]`. Using this model, generate propensity scores as a new variable called `ps` in `school`. (**0 Points**)

```{r}
# Your code here...
```

`r q <- q+1; letters[q]`. Using these propensity scores, create inverse probability weights as a new variable called `w` in `school`. (**1 Point**)

```{r}
# Your code here...
```

`r q <- q+1; letters[q]`. Using these weights, re-create the income distributions from before.  Comment on any similarities/differences.^[Hint: `density()` contains an option for weights.] (**1 Point**)
...
`r q <- q+1; letters[q]`. First estimate the models' parameters using OLS and store them as `r_ols1` and `r_ols2`. You do not need to display the results quite yet. (**1 Point**)

```{r}
# Your code here...
```

`r q <- q+1; letters[q]`. Use the `MatchIt` package in R to create a matched sample (match based on either Mahalanobis distance or propensity scores -- up to you).  Then, estimate the parameters with OLS using the matched sample. Store these as `r_match1` and `r_match2`. Again, hold off on displaying these coefficients. (**1 Point**)

```{r}
# Your code here...
```

`r q <- q+1; letters[q]`. Estimate the parameters with OLS using the inverse probability weights you generated before. Store these estimations as `r_ipw1` and `r_ipw2`. (**1 Point**)

```{r}
# Your code here...
```

`r q <- q+1; letters[q]`. Display the coefficients of the above six models in a table generated by `modelsummary`. (**1 Point**)
