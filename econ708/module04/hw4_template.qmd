...
Estimate the following models linking catholic school enrollment and math scores.  The functional forms are:
...
`r q <- q+1; letters[q]`. Estimate the model parameters with OLS. (**1 Point**)

```{r}
# Your code here...
```

`r q <- q+1; letters[q]`. Use the `MatchIt` package in R to create a matched sample (use `method = "nearest"`).  Then, estimate the parameters with OLS using the matched sample. (**1 Point**)

```{r}
# Your code here...
```

`r q <- q+1; letters[q]`. Estimate the parameters OLS and inverse probability weights. (**1 Point**)

```{r}
# Your code here...
```

`r q <- q+1; letters[q]`. Display the coefficients of these models in a table generated by `modelsummary`. (**1 Point**)
